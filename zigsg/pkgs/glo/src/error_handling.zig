...
pub fn loadCompileErrorMessage(handle: gl.GLuint) void {
    ...
    gl.getShaderInfoLog(handle, @intCast(c_int, error_buffer.len), &size, @ptrCast(*u8, &error_buffer[0]));
...
pub fn loadLinkErrorMessage(handle: gl.GLuint) void {
    ...
    var size: gl.GLsizei = undefined;
    gl.getProgramInfoLog(handle, @intCast(c_int, error_buffer.len), &size, @ptrCast(*u8, &error_buffer[0]));
    message = error_buffer[0..@intCast(usize, size)];
}
